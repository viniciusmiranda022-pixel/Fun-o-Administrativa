...
export const api = {
  ...
  dashboard: (tenantId) => client.get('/dashboard', { params: tenantId ? { tenantId } : {} }).then((r) => r.data),
  updateTenantBackupConfig: (tenantId, cfg) => client.put(`/tenants/${tenantId}/backup-config`, cfg).then((r) => r.data),
  runBackupFor: (tenantId) => client.post('/backups/run', { tenantId }).then((r) => r.data),
  unpackBackup: (backupId, opts) => client.post('/backups/unpack', { backupId, ...opts }).then((r) => r.data),
  refreshDifferences: (tenantId, backupId) => client.post('/compare/run', { tenantId, backupId }).then((r) => r.data),
  exportEvents: (severity) => client.get('/events/export', { params: severity ? { severity } : {}, responseType: 'blob' }).then((r) => r.data),
